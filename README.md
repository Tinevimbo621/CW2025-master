...
* **[⚠Unexpected Problems & Solutions](#⚠️-unexpected-problems--solutions)**
...
* **Setup:** Ensure you have already configured the JavaFX SDK as a library (see [🛠️ IDE Setup](#-ide-setup-configuration)).
* **Steps:**
...
#### 🎮 Core Gameplay
* **Brick spawning:** ✅ Generates a new tetromino at the top of the board using a queue system.
* **Brick movement:** ✅ Allows horizontal and vertical movement based on user input and gravity.
* **Collision detection:** ✅ Prevents illegal movement into walls, the floor, or other bricks.
* **Brick merging:** ✅ When a piece lands, it is locked into the board grid.
* **Soft drop + scoring:** ✅ Player-controlled downward movement awards small score bonuses.
* **Hard drop + multiplier:** ✅ Instantly drops the brick to its ghost position and applies score multiplier.
* **Rotation system:** ✅ Allows rotating any tetromino clockwise or counter-clockwise.
* **Row clearing:** ✅ Detects and clears completed lines, shifting upper rows downward.
* **Combo scoring:** ✅ Awards extra points for clearing multiple rows consecutively.
* **Next-brick preview (x3):** ✅ Displays the next three tetrominoes in queue.
* **Hold system:** ✅ Lets players store one tetromino for later use (with swap restrictions).
* **Game over detection:** ✅ Triggers when new bricks cannot spawn safely.
* **Board rendering:** ✅ Draws the board grid and bricks using JavaFX Canvas.
...
* **Ultra Mode**
    ...
    * ✅ **Countdown timer:** 180-second global timer for the entire session.
    * ✅ **Continuous play:** No line targets; gameplay flows until timer ends.

#### 🖥 UI & Rendering
* ✅ **Grid renderer:** Draws the playfield matrix, including locked bricks.
* ✅ **Ghost piece rendering:** Shows where the current brick will land.
* ✅ **Held brick renderer:** Renders the stored tetromino in the hold box.
...
* ✅ **ViewData model:** Packs board and brick state into a single transferable model.
* ✅ **Modular components:** Dedicated classes handle movement, ghost logic, timers, etc.
* ✅ **Input event system:** Cleanly maps user input to game actions.
...
These features are functional but have minor, known issues (short descriptions included for clarity):

* **Sprint level-up timing drift:** Level sometimes changes a tick early due to timer alignment.
* **Ultra mode timer reset edge case:** Timer does not always refresh visually when pressing `N` (New Game).
* **Initial rotation delay:** First brick previously could not rotate for 1–2 frames (fixed during refactor).
* **Ghost offset discrepancies:** Ghost sometimes off by ±1 row for extremely wide shapes or unusual spawn positions.
...
* **Sound settings:** No sound/music toggle menu.
* **Settings persistence:** Options (if any were added) would reset on restart.
...
## ⚠️ Unexpected Problems & Solutions
